...
dimension on the Breast Cancer Wisconsin Diagnostic dataset using 5-fold cross-validation. The recovered
experimental results show that the hybrid student with KD reaches AUC $0.9931 \pm 0.0054$, matching the
KD classical student in AUC, while the hybrid student without KD reaches the best hybrid F1
($0.9442 \pm 0.0220$). However, the hybrid models require $276$--$330$ seconds per fold, compared with
less than one second for the classical students, so the performance gains are not sufficient to justify
the extra simulation cost on this dataset. Ablation experiments show that front placement of the quantum
layer fails badly, whereas middle and tail placement remain competitive; PCA dimensions 6--8 improve
F1/MCC over 4 dimensions, with 6 dimensions offering the best AUC-cost balance. These results suggest
that lightweight hybrid models are feasible for biomedical tabular learning, but careful placement and
compression design matter more than simply adding a quantum layer.
\end{abstract}
...
already available~[CITE]. Hybrid quantum-classical (HQC) models therefore remain the most realistic
design pattern: a classical front-end performs feature compression, while a small parameterized quantum
circuit (PQC) acts as a learnable nonlinear block~[CITE].
...
5-fold protocol. Second, we ablate quantum layer placement and PCA dimension. Third, we report both
predictive quality and training cost, which makes the trade-off visible rather than relying on accuracy
alone.
...
component small enough for near-term simulation or deployment~[CITE]. Prior work has explored this idea
for image, text, and tabular settings, but biomedical tabular benchmarks remain comparatively underused
despite their natural low-dimensional structure after feature selection or PCA~[CITE].
...
teacher can regularize difficult hybrid optimization~[CITE].
...
bounded by the number of available qubits~[CITE]. This makes WDBC a suitable benchmark for controlled
HQC experiments.
...
\texttt{BasicEntanglerLayers}. For 4 qubits and 1 variational layer, the effective circuit depth is 2
(embedding + one entangling block). The circuit returns 4 Pauli-$Z$ expectation values that are passed
to the classical head.
...
\begin{table}[t]
\caption{Main student results on WDBC (5-fold CV, default setting with PCA $d'=4$).}
\label{tab:main}
\centering\small
\begin{tabular}{lcccc}
\toprule
...
Table~\ref{tab:main} shows that the classical student is already very strong on WDBC. The hybrid student
without KD slightly trails the classical student in AUC but matches it in F1 and MCC. Adding KD to the
hybrid student raises AUC from $0.9896$ to $0.9931$ and reduces mean training time by about 16\%, but
it slightly reduces F1 and MCC. In contrast, KD does not improve the classical student on this benchmark.
...
The placement study in Table~\ref{tab:ablation_pos} reveals the most important design choice in this
paper: placing the quantum block too early is harmful. Front placement collapses predictive quality,
whereas middle and tail placement both remain competitive. Tail placement yields the best AUC and MCC,
while middle placement gives the best F1. Because tail placement is also faster on average than middle,
it provides the strongest overall placement trade-off.

\begin{figure}[t]
\centering
\includegraphics[width=0.88\linewidth]{ablation_position.pdf}
\caption{Effect of quantum layer placement. Front placement is unstable, while middle and tail remain
high-performing.}
...
shows that $T=4$ gives a marginally higher AUC, but $T=2$ is preferable overall because it yields better
F1 and MCC with less variance.
...
model improves AUC and lowers training time, which suggests that teacher guidance stabilizes optimization.
However, the same model slightly loses F1 and MCC. On WDBC, this means KD is useful when AUC is the
priority, but it does not produce a uniformly better hybrid classifier.

\subsection{Which quantum layer position is best?}
The placement ablation makes it clear that the quantum layer should not be applied directly to the raw
PCA vector in this setting. A small classical projection before the quantum block is essential. Both
middle and tail placement work well, with tail placement offering a slightly better performance-cost
balance. This is a more actionable design lesson than the binary question of whether a quantum layer is
present at all.

\subsection{Is the hybrid student worth the cost?}
For this dataset, the answer is generally no. The classical student already reaches AUC above 0.994 with
sub-second training per fold. The hybrid students can remain competitive, but they are two to three
...
\texttt{default.qubit}. We do not evaluate shot noise, hardware noise, calibration drift, or larger
cohorts. In addition, the recovered results are based on fold-aggregated notebook outputs rather than
full prediction archives, so calibration and per-sample error analysis are outside the current scope.
...
than the mere presence of a quantum block. These findings support a pragmatic view of HQC modeling:
careful architectural placement is necessary, and strong classical baselines remain hard to outperform on
small biomedical tabular benchmarks.
